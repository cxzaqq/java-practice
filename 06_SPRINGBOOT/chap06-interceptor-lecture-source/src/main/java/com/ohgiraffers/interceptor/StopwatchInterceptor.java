...
*   인터셉터를 사용하는 경우(목적)
*   : 로그인 체크, 권한 체크, 프로그램 실행 시간 계산 작업 로그 처리, 업로드 파일 사이즈 처리(리사이즈), 로케일(지역) 설정 등 */
@Component
public class StopwatchInterceptor implements HandlerInterceptor {
    private final InterceptorTestService testService;
    ...
    @Override
    /* 설명. 경우에 따라 컨트롤러가 동작하지 않게 해주기 위해 boolean을 반환함 */
